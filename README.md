...
git clone https://github.com/your-username/api-address-validation.git
cd api-address-validation
...
- **Base URL**: http://localhost:8080/v1/address/
...
```bash
curl -H "Authorization: Bearer dev-address-key" \
     http://localhost:8080/v1/address/endpoint
```
...
curl -H "X-RapidAPI-Proxy-Secret: your-secret" \
     ...
     https://your-api.p.rapidapi.com/v1/address/endpoint
```
...
```bash
# Build image
docker build -t api-address-validation .

# Run container
docker run -p 8080:8080 \
  -e ADDRESS_VALIDATION_API_KEY=your-key \
...
2. Repository: `your-username/api-address-validation`
...
| `ADDRESS_VALIDATION_API_KEY` | Service API key | dev-address-key |
...
ADDRESS_VALIDATION_API_KEY=your-development-key
```
...
ADDRESS_VALIDATION_API_KEY=your-production-api-key
...
2. **Docker Build Issues**
   ...
   docker build --no-cache -t api-address-validation .
   ```

3. **Environment Issues**
   ```bash
   # Check environment variables
   docker-compose logs address-validation-api
   ```
